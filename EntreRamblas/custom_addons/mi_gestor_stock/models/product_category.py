...
Crear/reutilizar por nombre (`mgs_find_or_create`, usada por
`mgs_category_widget.js` en recepción y en la ficha de producto) es un asunto
aparte, resuelto con `mgs_name_normalized` más abajo: que escribir "Rosas",
"rosas " o "ROSAS" siempre resuelva a la MISMA categoría en vez de crear una
nueva cada vez.
"""
import logging
import re
...
class ProductCategory(models.Model):
    ...
    # ==================================================================
    # Crear/reutilizar por nombre (widget de autoguardado)
    # ==================================================================
    @api.depends("name")
    ...
    def mgs_find_or_create(self, name):
        ...
        Una categoría creada aquí pasa por el `create` de arriba como
        cualquier otra: su botón en el TPV (`mgs_pos_categ_id`) aparece solo,
