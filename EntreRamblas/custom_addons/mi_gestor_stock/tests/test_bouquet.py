...
class TestBouquet(TestPointOfSaleCommon):
    ...
    def test_recipe_validates_against_the_same_parser_as_checkout(self):
        """Una receta guardada nunca puede contener algo que luego el cobro
        rechace: se valida con el MISMO parser (_mgs_parse_components)."""
        recipe = self.env["mgs.bouquet.recipe"].create({
            "name": "Ramo de prueba clasico", "list_price": 30.0,
            "line_ids": [Command.create({"product_id": self.rose.id, "quantity": 7}),
                         Command.create({"product_id": self.green.id, "quantity": 2})],
        })
        self.assertEqual(json.loads(recipe.spec_json),
                         [{"product_id": self.rose.id, "qty": 7.0},
                          {"product_id": self.green.id, "qty": 2.0}])
        # Un material que no lleva existencias (una composicion) no puede
        # formar parte de una receta, igual que no puede formar parte de un
        # ramo montado a mano.
        with self.assertRaises(UserError), self.env.cr.savepoint():
            self.env["mgs.bouquet.recipe"].create({
                "name": "Receta imposible",
                "line_ids": [Command.create({"product_id": self.bouquet.id, "quantity": 1})],
            })
        with self.assertRaises(UserError), self.env.cr.savepoint():
            self.env["mgs.bouquet.recipe"].create({"name": "Receta vacia"})

    def test_only_the_owner_manages_recipes(self):
        staff = new_test_user(self.env(context=dict(self.env.context, no_reset_password=True)),
            login="recipe_staff", groups="mi_gestor_stock.group_mgs_user",
            company_id=self.env.company.id)
        with self.assertRaises(AccessError), self.env.cr.savepoint():
            self.env["mgs.bouquet.recipe"].with_user(staff).create({
                "name": "Receta de la dependienta",
                "line_ids": [Command.create({"product_id": self.rose.id, "quantity": 1})],
            })
        recipe = self.env["mgs.bouquet.recipe"].create({
        ...
        with self.assertRaises(AccessError), self.env.cr.savepoint():
            recipe.with_user(staff).write({"list_price": 99})

    def test_recipe_spec_json_updates_when_lines_change(self):
        recipe = self.env["mgs.bouquet.recipe"].create({
            "name": "Receta ajustable",
            "line_ids": [Command.create({"product_id": self.rose.id, "quantity": 5})],
        })
        self.assertEqual(json.loads(recipe.spec_json), [{"product_id": self.rose.id, "qty": 5.0}])
        recipe.line_ids.write({"quantity": 9})
        self.assertEqual(json.loads(recipe.spec_json), [{"product_id": self.rose.id, "qty": 9.0}])
